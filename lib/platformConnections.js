...
} from "./tokenEncryption.mjs";

async function decryptAndMigrateConnection(admin, connection) {
  if (!connection) return null;

  const decrypted = decryptConnectionTokens(connection);
  const shouldMigrate =
    tokenEncryptionConfigured() &&
    ((connection.access_token && !isEncryptedSecret(connection.access_token)) ||
      (connection.refresh_token && !isEncryptedSecret(connection.refresh_token)));

  if (shouldMigrate) {
    const encrypted = encryptTokenFields({
    ...
    if (error) {
      throw new Error(`Failed to migrate provider credentials: ${error.message}`);
    }
  }
...
export async function getPlatformConnection({
  ...
  const connection = await decryptAndMigrateConnection(admin, data || null);
...
export async function findPlatformConnection({
  ...
  return decryptAndMigrateConnection(admin, data || null);
}

export async function getPlatformConnectionForWorkspace({
  workspaceId,
  provider,
  accountId = "",
  requireConnected = true
...
export async function upsertPlatformConnection({
  ...
  if (existing?.id) {
    ...
    return {
      user,
      workspace,
      connection: decryptConnectionTokens(data)
    };
  }
...
export async function updatePlatformConnection({
  ...
  const safeUpdates = encryptTokenFields(updates);
