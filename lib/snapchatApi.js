...
export function getDateRange(preset) {
  ...
  const today  = {
    ...
    h:rNow.getUTCHours(), min:rNow.getUTCMinutes()
  };
  function shiftDay(n) {
  ...
  function stamp(p,h,min=0) { return `${p.y}-${pad(p.m)}-${pad(p.d)}T${pad(h)}:${pad(min)}:00.000+03:00`; }
  const nowH = today.h;
  const nowMin = today.min;
  let startDay, endDay, endHour, endMinute=0;
  if      (preset==="today")      { startDay=today;           endDay=today; endHour=nowH; endMinute=nowMin; }
  else if (preset==="yesterday")  { startDay=shiftDay(-1);    endDay=today; endHour=0;    }
  else if (preset==="last_7d")    { startDay=shiftDay(-6);    endDay=today; endHour=nowH; endMinute=nowMin; }
  else if (preset==="last_30d")   { startDay=shiftDay(-29);   endDay=today; endHour=nowH; endMinute=nowMin; }
  else if (preset==="this_month") { startDay={...today,d:1};  endDay=today; endHour=nowH; endMinute=nowMin; }
  else if (preset==="last_90d")   { startDay=shiftDay(-89);   endDay=today; endHour=nowH; endMinute=nowMin; }
  else if (preset==="maximum")    { startDay=shiftDay(-364);  endDay=today; endHour=nowH; endMinute=nowMin; }
  else                            { startDay=shiftDay(-29);   endDay=today; endHour=nowH; endMinute=nowMin; }
  return { startTime:stamp(startDay,0), endTime:stamp(endDay,endHour,endMinute) };
}
...
// Snapchat exposes aggregate delivery and conversion metrics at ad-account
// level. This is the authoritative and fast source for the dashboard total,
// and avoids hundreds of per-campaign requests for a single Overview row.
