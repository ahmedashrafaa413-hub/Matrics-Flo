...
export function parseAsyncStatsCsv(text, level, expectedEntityIds = []) {
  ...
  // Snapchat CSV downloads can start with report metadata/preamble rows.
  // Discover the actual header instead of assuming the first row is it.
  const normalizedRows = csvRows.map((row) => row.map(normalizeCsvHeader));
  let headerRowIndex = normalizedRows.findIndex((row) =>
    idCandidates.some((candidate) => row.includes(candidate))
  );

  // Snapchat occasionally labels the entity column differently in downloaded
  // campaign reports. Locate the metric header row, then identify the entity
  // column by matching its values against IDs returned by the metadata API.
  // This is safer than accepting a name-like column or relying on undocumented
  // CSV labels, and guarantees that every stats row maps to a real Ad Squad.
  const expectedIdSet = new Set(expectedEntityIds.map(String));
  if (headerRowIndex < 0 && expectedIdSet.size) {
    const metricHeaderRowIndex = normalizedRows.findIndex((row) =>
      row.some((header) => metricFields.has(header))
    );

    if (metricHeaderRowIndex >= 0) headerRowIndex = metricHeaderRowIndex;
  }

  if (headerRowIndex < 0) {
    // An async report with no delivery can contain only a status/preamble row.
    // Treat that as an empty report, but retain a useful error when Snapchat
    // returned metric columns with an unexpected entity identifier.
    const hasMetricHeader = normalizedRows.some((row) =>
      row.some((header) => metricFields.has(header))
    );
    if (!hasMetricHeader) return [];
  ...
  let idIndex = idCandidates
    .map((candidate) => headers.indexOf(candidate))
    .find((index) => index >= 0);

  if (idIndex === undefined && expectedIdSet.size) {
    const dataRows = csvRows.slice(headerRowIndex + 1);
    idIndex = headers.findIndex((_, columnIndex) =>
      dataRows.some((row) => expectedIdSet.has(String(row[columnIndex] || "").trim()))
    );
    if (idIndex < 0) idIndex = undefined;
  }
  ...
  return csvRows.slice(headerRowIndex + 1).flatMap((values) => {
...
export async function fetchBreakdownStats({
  ...
  // Snapchat only permits an Ad Squad breakdown on a Campaign stats endpoint.
  // Run one async report per parent Campaign, with bounded concurrency, then
  // combine the complete reports. Ad-account stats reject this as E1004.
  if (level === "adsquad") {
    if (!campaignIds.length) return { ok: true, status: 200, rows: [] };

    const reports = await mapWithConcurrency(campaignIds, 4, (campaignId) =>
      fetchAsyncBreakdownReport({
        ownerPath: `campaigns/${encodeURIComponent(campaignId)}`,
        breakdown: "adsquad",
        parseLevel: "adsquad",
        token,
        startTime,
        ...
        swipeWindow,
        viewWindow,
        expectedEntityIds: entityIdsByCampaign[campaignId] || []
      })
    );
  ...
  return fetchAsyncBreakdownReport({
    ownerPath: `adaccounts/${encodeURIComponent(accountId)}`,
    breakdown: level === "ad" ? "ad" : "campaign",
    parseLevel: level,
    token,
    startTime,
    endTime,
...
async function fetchAsyncBreakdownReport({
  ownerPath,
  breakdown,
  parseLevel,
  token,
    ...
    if (status === "COMPLETED" && report.result) {
      try {
        ...
        return {
          ok: true,
          status: 200,
          rows: parseAsyncStatsCsv(csv, parseLevel, expectedEntityIds),
