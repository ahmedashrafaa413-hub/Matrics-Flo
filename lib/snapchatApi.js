...
// Snapchat restricts a direct ad-account TOTAL query to the spend field.
// Requesting a campaign breakdown is still a single API call, but allows all
// delivery and conversion fields. Sum the raw campaign stats first so ratio
// metrics (ROAS, CPA, CTR, CPC) are recomputed correctly from account totals.
export async function fetchAccountStats(args) {
  const result = await fetchBreakdownStats({
    ...args,
    level: "campaign"
  });
  ...
  const stats = result.rows.reduce((totals, row) => {
