...
// Overview, fetch every ACTIVE campaign's authoritative stats and sum the raw
...
export async function fetchAccountStats(args) {
  ...
  // Snapchat can attribute purchases to campaigns that were paused after the
  // conversion happened. Summing only campaigns whose current status is ACTIVE
  // under-counts purchases and revenue compared with Ads Manager. Exclude only
  // deleted/archived entities; include ACTIVE, PAUSED and transitional states.
  const reportableCampaigns = entitiesResult.entities.filter(
    (entity) => !["DELETED", "ARCHIVED"].includes(entity.status)
  );
  const uniqueCampaigns = Array.from(
    new Map(reportableCampaigns.map((entity) => [entity.id, entity])).values()
  );
  const [results, accountSpendResult] = await Promise.all([
    mapWithConcurrency(
    uniqueCampaigns,
    6,
    (entity) =>
  ...
  if (failed.length || !accountSpendResult.ok) {
    return {
      ok: false,
      status: failed[0]?.status || 502,
      error: {
        message: failed.length
          ? `${failed.length} reportable campaign stat request(s) failed; the previous complete total was kept.`
          : "The official Snapchat ad-account spend request failed; the previous complete total was kept.",
        first_error: failed[0]?.error || accountSpendResult.error || null
      }
    };
  }

  const stats = results.reduce((totals, result) => {
  ...
  // Campaign endpoints provide conversion metrics, while Snapchat only allows
  // spend at ad-account total level. Use that authoritative account value so
  // Overview spend exactly follows Ads Manager and ratios use the same base.
  ...
  return {
    ...
    campaigns_counted: uniqueCampaigns.length,
    action_report_time: ACTION_REPORT_TIME
  };
}
...
export async function fetchBreakdownStats({
  ...
  const url =
    ...
    `&swipe_up_attribution_window=${swipeWindow}&view_attribution_window=${viewWindow}` +
    `&action_report_time=${ACTION_REPORT_TIME}` +
    `&omit_empty=true&limit=1000`;

  const result = await snapFetch(url, token);
  if (!result.ok) {
    return { ok: false, status: result.status, error: result.data || result.raw || null, rows: [] };
  }

  return {
    ok: true,
    status: result.status,
    rows: extractBreakdownStats(result.data, breakdown)
  };
}
