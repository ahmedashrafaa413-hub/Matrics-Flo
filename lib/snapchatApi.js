...
export async function fetchAccountStats(args) {
  ...
  const activeCampaigns = entitiesResult.entities.filter(
  ...
  if (failed.length) {
    return {
      ...
      error: {
        message: `${failed.length} active campaign stat request(s) failed; the previous complete total was kept.`,
  ...
  return {
    ...
    campaigns_counted: activeCampaigns.length
  };
}
