...
async function getWorkspaceConnectionToken(request, accountId = "") {
  const accountCandidates = accountId
    ? [accountId, "snapchat_default"]
    : ["snapchat_default"];

  for (const candidateAccountId of accountCandidates) {
...
async function getCookieTokenFallback() {
...
export async function getSnapchatToken(request = null, accountId = "") {
  if (request) {
    try {
      const workspaceToken = await getWorkspaceConnectionToken(
        request,
        accountId
      );
      ...
      // Keep cookie fallback during migration.
    }
  }

  return getCookieTokenFallback();
}
