...
// ── BULK breakdown — 1 call gets ALL entities ─────────────────────────────────
// Confirmed path: total_stats[0].breakdown_stats[level]
// Each item: { id, type, stats: { conversion_purchases, spend, ... } }
async function fetchBreakdown({accountId,level,token,startTime,endTime,swipeWindow,viewWindow}) {
  ...
  let statsById={}, pages=0, nextUrl=url;
  ...
  while (nextUrl && pages<10) {
    ...
    // CONFIRMED path from actual API response analysis
    ...
    nextUrl = r.data?.total_stats?.[0]?.paging?.next_link ||
              r.data?.paging?.next_link || null;
    pages++;
  }
...
export async function GET(request) {
  ...
  // 3 parallel calls: account summary + entity list + bulk breakdown stats
  ...
  if (breakdownResult.ok && Object.keys(breakdownResult.statsById).length>0) {
    // Fast path: bulk breakdown worked — merge entity names with stats
    rows=allEntities
    ...
    // Breakdown returned empty or failed — surface the error clearly
    return NextResponse.json({
      success:false,
      error:"Breakdown endpoint returned no data",
      breakdown_error:breakdownResult.error||null,
      debug:{startTime,endTime,swipeWindow,viewWindow},
    });
  }
  ...
  const payload={
    ...
    breakdown_pages:breakdownResult.pages_fetched,
    summary,data:rows,
  };
