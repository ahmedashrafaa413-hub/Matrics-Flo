...
function SnapTable({ title, loading, rows, meta, onLoadAll, loadAllLoading }) {
  return (
      ...
      <div className="dash-table-head">
        <div>
          ...
          <p>
            {loading ? "Loading..." :
              meta
                ? `${rows.length} loaded • ${meta.active} active • ${meta.total} total`
                : `${rows.length} rows`}
          </p>
        </div>
        {meta?.activeOnly && meta?.total > meta?.active && onLoadAll && (
          <button
            onClick={onLoadAll}
            disabled={loadAllLoading}
            style={{
              background: "var(--glass-2)", color: "var(--text)",
              border: "1px solid var(--border-2)", padding: "8px 16px",
              borderRadius: "var(--radius-md)", fontSize: 12, fontWeight: 700,
              cursor: "pointer", fontFamily: "inherit"
            }}
          >
            {loadAllLoading ? "⏳ Loading all..." : `Load All ${meta.total} →`}
          </button>
        )}
      </div>
...
export default function SnapchatPage() {
  ...
  const [showAllAdsquads, setShowAllAdsquads] = useState(false);
  ...
  function buildDateParam(activeOnly = false) {
    return `date_preset=${datePreset}${activeOnly ? "&active_only=1" : ""}`;
  }
  ...
  async function loadAccounts() {
    try {
      ...
      const selected = list.find((a) => a.id === saved)?.id || list[0]?.id || "";
  ...
  async function loadCampaigns(selectedAccount = accountId, loadAll = showAllCamps) {
    if (!selectedAccount || campLoading) return;
    const activeOnly = !loadAll;
    const key = `${selectedAccount}|campaign|${buildDateParam(activeOnly)}`;
    if (campaignsRequestRef.current === key) return;
    campaignsRequestRef.current = key;
    setCampLoading(true); setError("");
    try {
      const data = await apiGet(
        `/api/snapchat/insights?account_id=${selectedAccount}&level=campaign&${buildDateParam(activeOnly)}`
      );
      setCampRows(data.data || []);
      setCampSummary(data.summary || null);
      setCampMeta({
        total: data.total_entities,
        active: data.active_entities,
        loaded: data.loaded_count,
        activeOnly: data.active_only,
        rateLimited: data.rate_limited,
      });
    } catch (err) {
      setCampRows([]); setCampSummary(null); setCampMeta(null);
      ...
      {activeTab === "campaigns" && (
        <SnapTable
          ...
          onLoadAll={() => {
            ...
            campaignsRequestRef.current = "";
            loadCampaigns(accountId, true);
          }}
          loadAllLoading={campLoading && showAllCamps}
        />
      )}
